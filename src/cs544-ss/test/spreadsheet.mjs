...
describe('persistent-spreadsheet', function() {
  ...
  //inspired by "bug" report from Y. Gaurav
  it('must copy cells repeatedly', async function() {
    const ss = spreadsheet;
    await ss.eval('a1', '3');
    assert.equal((await ss.query('a1')).value, 3);
    await ss.eval('k13', 'a1*3'); //k13 == a1*3 == 9
    assert.equal((await ss.query('k13')).value, 9);
    await ss.copy('k14', 'k13');  //k14 == a2*3 == 0
    assert.equal((await ss.query('k14')).value, 0);
    await ss.eval('a2', 'a1*2');  
    assert.equal((await ss.query('a2')).value, 6);
    assert.equal((await ss.query('k14')).value, 18);
  });

  //inspired by "bug" report from Y. Gaurav
  it('cannot copy cell which refers to "negative" cell refs', async function() {
    const ss = spreadsheet;
    await ss.eval('a1', '5');
    await ss.eval('a3', 'a1+3');
    assert.equal((await ss.query('a3')).value, 8);
    try {
      await ss.copy('a2', 'a3'); //copied formula is a0*3; invalid cell ref a0
      const {formula, value: v} = await ss.query('a2');
      assert.fail(`expected SYNTAX error but copied ${formula}==${v}`);
    }
    catch(err) {
      if (!(err instanceof AppError)) assert.fail(err.message);
      assert.equal(err.code, 'SYNTAX');
